...
onready var initial_pos_local := translation
onready var initial_pos_global := global_transform.origin
onready var button_forward_vector_norm = global_transform.basis.z.normalized()
...
func _process(delta):
	...
	if touching:
		...
		var new_origin = Vector3(initial_pos_local.x, initial_pos_local.y, translation.z - dist)
